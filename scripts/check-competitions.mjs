...
const officialUrls = JSON.parse(await readFile('app/data/competition-official-urls.json', 'utf8'))
...
for (const [id, url] of Object.entries(officialUrls)) {
  assert.ok(rowIds.has(id), `official URL points to unknown row ${id}`)
  assert.match(url, /^https?:\/\/\S+$/, `invalid official URL for row ${id}`)
}
...
console.log(`Competition source OK: 253 rows, six categories, ${Object.keys(officialUrls).length} official URL mappings, 27-page source hash, page mapping and unknown audiences preserved.`)
