...
const destination = resolve('.cache/schedule-release-20260924-1')
...
for (const name of ['login-template-adapters.mjs', 'login-template-http.mjs', 'login-template-store.mjs', 'school-schedule.mjs', 'school-grades.mjs', 'academic-store.mjs']) await copyFile(`server/utils/${name}`, `${destination}/server/utils/${name}`)
