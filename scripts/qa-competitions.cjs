...
(async () => {
  ...
  try {
    ...
    assert.equal(await items.getByText('参考官网链接', { exact: true }).count(), 1);
    assert.equal(await items.locator('a').count(), 1);
    assert.equal(await items.locator('a').getAttribute('href'), 'https://www.hbskills.org.cn/');
    await page.keyboard.press('Enter');
