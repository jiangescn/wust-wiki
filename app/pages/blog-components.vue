  ...
  <main v-if="page" class="blog-components blog-showcase">
    ...
    <div class="showcase-layout">
      ...
      <details v-if="page.body.toc?.links?.length" class="showcase-toc showcase-mobile-toc">
        ...
        <nav aria-label="本页目录"><a v-for="item in page.body.toc.links" :key="item.id" :href="`#${item.id}`">{{ item.text }}</a></nav>
      </details>
      ...
      <aside class="showcase-aside" aria-label="示例目录与侧栏插槽">
        <details v-if="page.body.toc?.links?.length" class="showcase-toc showcase-desktop-toc" open><summary>本页目录</summary><nav aria-label="本页目录"><a v-for="item in page.body.toc.links" :key="item.id" :href="`#${item.id}`">{{ item.text }}</a></nav></details>
...
@media(max-width:1024px) {
  ...
  .showcase-aside { position:static;max-height:none;width:100%;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem; }
