  ...
  <main v-if="page" class="blog-components blog-showcase">
    ...
    <div class="showcase-layout">
      ...
      <aside class="showcase-aside" aria-label="示例目录与侧栏插槽">
        <details open><summary>本页目录</summary><nav><a v-for="item in page.body.toc?.links" :key="item.id" :href="`#${item.id}`">{{ item.text }}</a></nav></details>
...
.showcase-layout { display:grid;grid-template-columns:minmax(0,1fr) 260px;gap:2rem; }.showcase-main { min-width:0; }.showcase-aside { min-width:0;align-self:start;position:sticky;top:5rem;max-height:calc(100dvh - 6rem);overflow:auto; }.showcase-aside nav { display:grid;gap:.6rem;margin:1rem 0; }.showcase-aside { font-size:.85rem; }.showcase-aside h2 { font-weight:600; }
.showcase-aside > section { margin:1rem 0;padding:1rem;overflow-wrap:anywhere; }
.showcase-aside :deep(.link-card) { max-width:100%; }
@media(max-width:1024px) { .showcase-layout { display:flex;flex-direction:column; }.showcase-aside { order:-1;position:static;max-height:none;width:100%;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem; }.showcase-aside > details { grid-column:1/-1; }.showcase-aside > section { margin:0; } }
